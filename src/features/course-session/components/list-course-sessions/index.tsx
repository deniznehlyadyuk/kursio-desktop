...
import { ListCourseSessionsProps } from './types';
import useListCourseSessions from './use-list-course-sessions';

export default (props: ListCourseSessionsProps) => {
  const [states, response, deleteButtonDisabled] = useListCourseSessions();
  ...
  const table = useMantineReactTable({
    ...
    renderRowActions: ({ row }) => (
      <RowActions
        onAttendancesButtonClick={() =>
          props.onAttendancesButtonClick(row.original)
        }
      />
    ),
